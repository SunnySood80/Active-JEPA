...
def calculate_jepa_rewards(episodes, jepa_outputs):
    ...
    # Transfer mask_indices batch to CPU once (if it's a tensor)
    if torch.is_tensor(jepa_outputs['mask_indices']):
        mask_indices_cpu = jepa_outputs['mask_indices'].detach().cpu()
    else:
        mask_indices_cpu = jepa_outputs['mask_indices']
    
    for i, episode in enumerate(episodes):
        episode_rewards = []
        pixel_errors = jepa_outputs['pixel_errors'][i]
        feature_maps = jepa_outputs['features'][i]
        mask_indices = mask_indices_cpu[i] if torch.is_tensor(mask_indices_cpu) else jepa_outputs['mask_indices'][i]
        
        # Map mask indices to patch positions (now on CPU)
        if torch.is_tensor(mask_indices):
            valid_indices = mask_indices[mask_indices >= 0].numpy()
        else:
            valid_indices = np.array(mask_indices)[np.array(mask_indices) >= 0]
        
        # Transfer feature_maps to CPU once per episode (not per action)
        ...
        for j, action in enumerate(episode['actions']):
            if j < len(pixel_errors) and j < len(valid_indices):
                ...
                # Semantic coherence (feature_maps already on CPU)
...
def calculate_semantic_coherence(feature_maps, action):
    try:
        # Assume feature_maps is already on CPU (transferred once per episode)
        if torch.is_tensor(feature_maps):
            # Only convert to float if needed, but don't transfer (should already be CPU)
            if feature_maps.device.type != 'cpu':
                feature_maps = feature_maps.detach().cpu().float()
            elif feature_maps.dtype != torch.float32:
